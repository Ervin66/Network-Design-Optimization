...
float ddu_headers[client_id][datetime] = ...; // order count per client per time period
...
dvar boolean EXW_boolean[client_id][wh_id][datetime]; // pallets to be picked up at wh j at time t
...
//objective
minimize
	sum(i in client_id, j in wh_id, t in datetime)Active_EXW*((EXW_boolean[i][j][t] + EXW_Backlog[i][j][t])*out_dist[i][j]*exw_ttkm[i][t]) //pick-up transporation costs
...
forall(i in client_id, t in datetime)(sum(j in wh_id)(EXW_boolean[i][j][t]*exw_orders[i][t] + EXW_Backlog[i][j][t]*exw_orders[i][t])) ==  Active_EXW*exw_orders[i][t]; 
...
InboundCt:
forall(t in datetime, w in warehouse)sum(p in production) Active_Inbound*Inb_Pallet[p][w][t] == sum(i in client_id)Pallet_Trans[i][w][t];
TransCostsCt:
forall(i in client_id, j in wh_id)sum(t in datetime)((DDU_boolean[i][j][t]+DDU_Backlog[i][j][t])*(out_dist[i][j]*ddu_ttkm[i][t] 
																	 + ddu_fix*ddu_headers[i][t]))  == DDU_Trans_Costs[i][j];
...
forall(i in client_id, j in wh_id)sum(t in datetime)(DDU_Backlog[i][j][t]*ddu_orders[i][t] + EXW_Backlog[i][j][t]*exw_orders[i][t])*Active_Backlog  == Backlog[i][j];
